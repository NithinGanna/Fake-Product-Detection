...
const ExpiryAlerts = () => {
  ...
  const [alertAdded, setAlertAdded] = useState(false); // State for alert addition status
  ...
  const handleAddAlert = async () => {
	try {
	  ...
	  const response = await axios.get('http://localhost:5000/api/getApi', {
	  ...
	  const expiryDate = new Date(Number(product[6]));
	  ...
	  const alertData = {
		...
		reminderDate: reminderDate.toISOString() // Convert to ISO string format
	  };
  ...
  return (
    <>
      ...
      <div className="container mx-auto text-center">
        <h1 className="text-3xl font-bold my-8">Alert Page</h1>
        {!isCameraActive ? (
          <>
            <button
        ...
        {product && (
          <div className="border p-4 my-4">
            ...
            {/* Display feedback based on alert addition status */}
            {alertAdded && (
              <p className="text-green-500 font-semibold mt-2">Alert added successfully!</p>
            )}
          </div>
        )}
      </div>
